...
Use `head()` to view the first 6 rows of the imported data:  
...
We can use the `readxl::read_excel()` function which reads xls or xlsx files. Here we use a path to an Excel file:  
...
Type `?read_excel` in your RStudio Console to see additional examples.  
...
When you run this code in RStudio, it will open the data in the RStudio's interactive data viewer, which is a spreadsheet-like viewer that provides a quick look at the data. You can also open the viewer by clikcing the dataset's name in the "Environment Pane":  
...
This viewer is especially useful for quickly scanning through your data, checking variable names, and seeing the values that your variables take. However, remember, this is only for viewing; you cannot edit your data in this viewer.  
...
In the next section, titled **"Data Wrangling"**, we will dive deeper into this topic, focusing on techniques for transforming and cleaning data. These skills are crucial in preparing data for analysis, making them a vital part of any data professional's toolkit.  
