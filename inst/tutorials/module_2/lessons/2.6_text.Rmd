...
The `stringr` package from the `tidyverse` offers a wide range of functions for manipulating text data. We will cover some of the most commonly used functions.  
...
For instance, consider a vector of employee feedback comments. View the `feedback_comments` vector in the codeblock:  
...
Let's focus on the dot `.` as an example. The `.` is a special character known as a **wildcard**. It matches any character except a newline. Let's assume we want to extract words that contain a period `(.)`. You might initially specify the `.` as the pattern:  
...
You'll notice that all the words were returned, not the words containing periods as you hoped. If you want to *literally* search for these metacharacters in a string, you must escape them using two backslashes `(\\)`. Let's add an escape double backslash to the pattern:  
...
//TODO: cheat sheet resource 

//TODO: resource for practicing regex online 
...
For instance, let's say we want to identify any mentions of a repeated weekly meeting in an email thread. A repeated meeting might be indicated by the word "week" followed by "ly" or "s".  
...
Using look arounds, we can extract job titles from a job posting:   
...
Phew! That wasn't to bad, right? Remember, regular expressions can get very complex and sometimes it takes a bit of trial and error to get them right. But once you master them, they can be extremely powerful tools in your data manipulation toolbox.  
